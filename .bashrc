...
export PATH=/home/koad/.meteor:$PATH
export LEGACY_HOME=/media/koad/2065a1af-a718-44f9-8903-8e1a8c6165c5/home/koad
export METEOR_PACKAGE_DIRS=$HOME/.beatify/packages:$HOME/.ecoincore/packages:$HOME/.koad-io/packages
...
# Check if /sbin and /usr/sbin are in PATH and add them if they're not
if ! echo $PATH | grep -q -E "(^|:)/sbin(:|$)"; then
    PATH="$PATH:/sbin"
fi

if ! echo $PATH | grep -q -E "(^|:)/usr/sbin(:|$)"; then
    PATH="$PATH:/usr/sbin"
fi
export PATH
...
[ -f $HOME/.ssh_aliases ] && source ~/.ssh_aliases
